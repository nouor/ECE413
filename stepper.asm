...
READ_PORT0: //READP0
	...
	//RRC A		// Checking the value of Port 0 to know if FS_SWITCH(P0.0) is pressed or not //8AH --->    1000 1011
																									  //1100 0101 --->C5H (Last bit 1 is copied to Cy bit)
	//JC HALF_STEP	//HALF_STEP	// If FS_SWITCH is not pressed then jump to HALF_STEP to check if HS_SWITCH is pressed //negative logic switch (0->pressed) (1-> not pressed)
...
FORMNUM:    				//ASSUME we enter 315 first A=3 then A=1 then A=5
	...
	MOV B,#10D				//3- B=10D
	MUL AB					//4- A*B,A=0B=0,A=30,B=0, A=36H,B=1H                                     A= LOWER BYTE, B=HIGHER BYTE,
	...
	ACALL INTERFACING_KEYPAD		// Get another key
...
KEY_VALUE:		// key value = ((row*N_col) + col + 1) = ((R2*3) + R0 +1)
	 MOV A,R2        //A=R2=1
	 MOV B,#3D       //N_COL =3 
	 MUL AB          //A= LOWER BYTE , B=HIGHER BYTE //A=3 B =0
	 MOV R2,A		 //R2=3	
	 MOV A,R0	     //A=0
	 ADD A,#1D       //1
	 ADD A,R2        //3+1=4
